...
class _UnlockDialogState extends State<UnlockDialog> {
  ...
  /// Returns the lockout duration in seconds for a given lockout cycle count.
  /// Cycle 1 → 60 s, cycle 2 → 5 min, cycle 3+ → 30 min (capped).
  static int _lockoutSeconds(int cycles) {
    if (cycles <= 1) return kLockoutSeconds; // 60 s
    if (cycles == 2) return 5 * 60;          // 5 min
    return 30 * 60;                          // 30 min cap
  }
  ...
  Future<void> _submit() async {
    ...
    var lockoutCycles = 0;
    try {
      // Load the persisted lockout-cycle counter for this folder.
      final cycleStr = await StorageService()
          .getSetting(StorageKeys.lockoutCount(widget.folder.id!));
      if (!mounted) return;
      lockoutCycles = int.tryParse(cycleStr ?? '0') ?? 0;

      // Check if still inside a previous lockout window.
      if (lockoutCycles > 0) {
        final attempts = await StorageService().getFailedAttempts(widget.folder.id!);
        if (!mounted) return;
        if (attempts.isNotEmpty) {
          final duration = _lockoutSeconds(lockoutCycles);
          final elapsed = DateTime.now().difference(attempts.first).inSeconds;
          if (elapsed < duration) {
            lockoutTriggered = true;
            _startLockout(duration - elapsed);
            return;
          }
        }
      }
      ...
      if (pinMatches || masterMatches) {
        ...
        StorageService()
            ...
            .ignore();
        StorageService()
            .setSetting(StorageKeys.lockoutCount(widget.folder.id!), '0')
            .catchError((e) => debugPrint('reset lockoutCount failed: $e'))
            .ignore();
        ...
        if (updated.count >= kMaxPinAttempts) {
          final newCycles = lockoutCycles + 1;
          await StorageService().setSetting(
              StorageKeys.lockoutCount(widget.folder.id!), newCycles.toString());
          lockoutTriggered = true;
          _startLockout(_lockoutSeconds(newCycles));
        } else {
          setState(() => _hasError = true);
        }
      }
